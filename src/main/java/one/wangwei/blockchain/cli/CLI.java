...
//    }
    ...
    private void printTransaction(Transaction tx) {
        System.out.println("  TX ID: " + org.apache.commons.codec.binary.Hex.encodeHexString(tx.getTxId()));

        System.out.println("    Inputs:");
        for (one.wangwei.blockchain.transaction.TXInput input : tx.getInputs()) {
            String txId = input.getTxId() == null || input.getTxId().length == 0
                    ? ""
                    : org.apache.commons.codec.binary.Hex.encodeHexString(input.getTxId());

            String scriptSig;

            //核心修改在这里
            if (input.getTxId() == null || input.getTxId().length == 0) {
                // Coinbase交易：这里存的是字符串（Reward to ...）
                scriptSig = input.getPubKey() == null ? "" : new String(input.getPubKey());
            } else {
                // 普通交易：pubKey是二进制 → 转Hex
                scriptSig = input.getPubKey() == null ? "" :
                        org.apache.commons.codec.binary.Hex.encodeHexString(input.getPubKey());
            }
        ...
        for (one.wangwei.blockchain.transaction.TXOutput output : tx.getOutputs()) {
            System.out.println("      Value: " + output.getValue()
                    + ", ScriptPubKey: " + org.apache.commons.codec.binary.Hex.encodeHexString(output.getPubKeyHash()));
        }
    }
